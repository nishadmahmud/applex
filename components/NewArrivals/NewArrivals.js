        ...
        <section className="w-full bg-gray-50 py-12 md:py-16">
            <div className="max-w-7xl mx-auto px-4 md:px-8">
                ...
                <div className="flex items-center justify-between mb-8 md:mb-10">
                    <div>
                        <h2 className="text-2xl md:text-3xl font-black text-gray-900 tracking-tight">Just Landed</h2>
                        <p className="text-applex-muted text-sm mt-1">The latest phones, fresh off the shelf</p>
                    </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-5">
                    {products.slice(0, 8).map((product) => (
                        <div key={product.id} className="group rounded-2xl bg-[#111] border border-gray-800 overflow-hidden hover:border-applex-cyan/50 transition-all duration-300 hover:shadow-xl hover:shadow-applex-cyan/10 flex flex-col">
                            {/* Image */}
                            ...
                            <div className="p-3 md:p-4 flex-1 flex flex-col">
                                {product.brand && (
                                    <span className="text-applex-cyan text-[10px] md:text-xs font-semibold uppercase tracking-wider mb-1">
                                        {product.brand}
                                    </span>
                                )}
                                <Link href={`/product/${product.name.toLowerCase().replace(/\s+/g, '-')}-${product.id}`}>
