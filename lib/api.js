 ...
 * @returns {Promise<Object>} Blog detail data
 */
export async function getBlogDetail(id) {
    if (!isApiConfigured()) return { success: true, data: null };
    const res = await fetch(
        `${process.env.NEXT_PUBLIC_API}/ecommerce-blog-detail/${id}`,
        { next: { revalidate: 60 } }
    );
    if (!res.ok) throw new Error(`Failed to fetch blog detail for ID ${id}`);
    return res.json();
}
